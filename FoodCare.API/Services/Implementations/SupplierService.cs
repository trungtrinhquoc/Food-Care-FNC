...
public class SupplierService : ISupplierService
{
    ...
    {
        var supplier = _mapper.Map<dat.Supplier>(createDto);
    ...
    public async Task<(IEnumerable<SupplierDto> Suppliers, int TotalCount)> GetSuppliersWithPaginationAsync(SupplierFilterDto filter)
    {
        ...
        {
            query = query.Where(s => 
                s.Name.ToLower().Contains(filter.SearchTerm.ToLower()) ||
                ...
                (s.ContactPerson != null && s.ContactPerson.ToLower().Contains(filter.SearchTerm.ToLower()))
            );
        }
