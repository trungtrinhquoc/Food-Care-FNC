...
import { ChevronLeft, MapPin, Star, Clock, Package, Loader2 } from 'lucide-react';
        ...
        <div className="min-h-screen bg-gray-50">
            <div className="bg-white border-b sticky top-0 z-40">
                <div className="container mx-auto px-4 py-3 max-w-6xl flex items-center gap-3">
                    ...
                    <p className="font-semibold text-gray-900">Trang Mart</p>
                </div>
            </div>
            ...
            <div className="container mx-auto px-4 py-5 max-w-6xl space-y-4">
                {loadingMart ? (
                    ...
                    <div className="bg-white rounded-2xl border overflow-hidden">
                        <div className="h-32 md:h-40 bg-gradient-to-r from-emerald-600 via-emerald-500 to-teal-500 relative">
                            {mart.storeBannerUrl && (
                                <img src={mart.storeBannerUrl} alt={mart.storeName} className="absolute inset-0 w-full h-full object-cover opacity-35" />
                            )}
                        ...
                        <div className="p-5 -mt-14 relative">
                            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        <h1 className="text-3xl font-bold text-gray-900">{mart.storeName}</h1>
                    ...
                    <div className="flex flex-col md:flex-row gap-2 md:items-center md:justify-between">
                        <div className="flex gap-2 flex-wrap">
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-full text-sm ${activeCategory === 'all' ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-700'}`}
                            >
                                Tất cả
                            </button>
                            {categories.map((c) => (
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-full text-sm ${activeCategory === c ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-700'}`}
                                >
                                    {c}
                                </button>
                            ))}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {loadingProducts ? (
                        <div className="col-span-full bg-white rounded-xl border p-6 text-gray-500 inline-flex items-center gap-2">
                            <Loader2 className="w-4 h-4 animate-spin" />
                            Đang tải sản phẩm...
                        </div>
                    ) : products.length === 0 ? (
                        ...
                        products.map((product) => {
                            ...
                            return (
                                <div key={product.id} className="bg-white rounded-xl border p-4 hover:shadow-md transition-shadow">
                                    <div className="aspect-square rounded-lg bg-gray-100 overflow-hidden mb-3 border">
                                        {img ? <img src={img} alt={product.name} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-gray-100" />}
                                    </div>
